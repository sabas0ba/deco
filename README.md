...
- **Seven settings deco ships defaults for are read by nothing yet:**
  `editor.renderControlCharacters`, `editor.tabCompletion`,
  `editor.largeFileOptimizations`, `files.autoSave`, `files.autoSaveDelay`,
  `files.encoding` and `workbench.editor.enablePreview`. Shipping a default for a key
  is a claim about it, so they are named here rather than left to be discovered.
